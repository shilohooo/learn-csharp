...
namespace Avalonia.DataTemplates.FuncDataTemplateSample.DataTemplates;

/// <summary>
/// </summary>
public static class DataTemplateProvider
{
    public static FuncDataTemplate<Person?> SexDataTemplate { get; } =
        new(
            // 检查是否匹配该模板
            person => person is not null,
            // Avalonia 会自动将 person 作为参数传入该方法
            // 相当于：person => BuildSexPresenter(person)
            BuildSexPresenter
        );
    ...
    /// <returns>性别展示控件实例</returns>
    private static Control BuildSexPresenter(Person? person)
    {
        var path = new Path
        {
            Width = 32,
